...
//  Clock
//
//  Created by Maksim Savin on 08.06.2021.
//
...
//        let sec = String(currentTime.sec)
        var sec: String {
            currentTime.sec < 10 ? String("0" + String(currentTime.sec)) :String(currentTime.sec)
        }
        let min = String(currentTime.min)
        let hour = String(currentTime.hour)
        let curTime = String(hour + ":" + min + ":" + sec)
